...
open import PTT-sub.Dom hiding (∈♦₀)
...
open import PTT-sub.Vars
open import PTT-sub.Conversion

module PTT-sub.Split where
...
TC-∈Split cont l (TC-?-inp (mk l₁ E/c) P) with sameVar? ([↦]∈.l… l) l₁
TC-∈Split {I = I} cont (mk l E/c') (TC-?-inp {c} (mk .l E/c) P) | same =
  cont-? cont refl ([↦…]∈.lI l) ([↦…]∈.lA l) E/c (λ m → P m)
TC-∈Split {I = I}{K}cont l (TC-?-inp (mk l₁ E/c) P) | diff x = TC-?-inp (∈♦₀ {I₁ = K} (move l (mk l₁ E/c) (mk x))) λ m →
  TC-conv
       (≈-trans (♦-cong₂ (≈-reflexive (/[>>]-/[>>]≡ I l₁ ([↦]∈.l… l) (Diff-sym… x))) ≈-refl)
                (≈-reflexive (! (∈♦₀-compute (move l (mk l₁ E/c) (mk x))))) )
    (TC-∈Split cont (move (mk l₁ E/c) l (mk (Diff-sym… x))) (P m))
TC-∈Split cont l (TC-!-out (mk l₁ E/c) m P) with sameVar? ([↦]∈.l… l) l₁
TC-∈Split cont (mk l E/c') (TC-!-out (mk .l E/c) m P) | same = TC-conv
  (♦-cong₂ (≈-reflexive ([≔][≔] _ _ (λ Δ → ≔'≔' Δ ([↦…]∈.lA l)) _ ([↦…]∈.lΔ l) )) ≈-refl) -- {!♦-cong₂ (≈,[end] _) ≈-refl!}
  (cont-! cont refl m (move-self _ (mk l E/c')) P)
TC-∈Split {I = I}{K} cont l (TC-!-out (mk l₁ E/c) m P) | diff x = TC-!-out (∈♦₀ {I₁ = K} (move l (mk l₁ E/c) (mk x))) m
  (TC-conv (≈-trans (♦-cong₂ (≈-reflexive (/[>>]-/[>>]≡ I l₁ ([↦]∈.l… l) (Diff-sym… x))) ≈-refl)
                    (≈-reflexive (! (∈♦₀-compute (move l (mk l₁ E/c) (mk x))))))
