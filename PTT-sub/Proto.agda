...
open import PTT-sub.Dom
open import PTT-sub.Session as Session hiding (Ended)
import PTT-sub.Map as Map
open Map using (Map; ε; _,_↦_; _↦_∈_; SelectionAll≡)
open import PTT-sub.Env as Env hiding (Ended; All; lookup; pure; zipWith)

module PTT-sub.Proto where
...
module _ {a}{A : Set a}{δ}(f g : Map A δ → Map A δ){h : Map A δ → Map A δ}(fg : ∀ Δ → g (f Δ) ≡ h Δ) where
  [≔][≔] : ∀ {δs}(I : Maps A δs)(l : [ δ ]∈D δs) → I [ l ≔ f ] [ l ≔ g ] ≡ I [ l ≔ h ]
...
[_/…_]≔_ : ∀ {δs}(I : Proto δs){c S}(l : [ c ↦ S …]∈ I) → MSession → Proto δs
[ I /… l ]≔ S = I [ lΔ >> lA ]≔ S
  where open [↦…]∈ l
...
{-
module _ {δ}{f g : 
[>>][>>]-red : ∀ {δI} (I : Proto δI)(l : [ δ ]∈D δI) → I [ l ≔ f ] [ l ≔ g ] ≡ I [ l ≔ g ∘ f ]
[>>][>>]-red = ?
-}
