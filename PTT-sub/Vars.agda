...
open import PTT-sub.Dom as Dom hiding (∈♦₀)
...
open import PTT-sub.Term

module PTT-sub.Vars where
...
∈♦₀-compute… : ∀ {δ₀ δ₁ c A S}{I₀ : Proto δ₀}{I₁ : Proto δ₁}(l : [ c ↦ A …]∈ I₀) →
          [(I₀ ♦Proto' I₁) /… (∈♦₀… l)]≔ S ≡ [ I₀ /… l ]≔ S ♦Proto' I₁
∈♦₀-compute… (mk lI lE) = ≔[∈]♦₀ lI
...
module _ {δI c d A B}S{I : Proto δI}{l : [ c ↦ A …]∈ I}{l' : [ d ↦ B …]∈ I} where
   -- need module because buggy Agda
   Env-move : DifferentVars… l l' → Env ([↦…]∈.δE l')
   Env-move (diff-ten x) = [↦…]∈.E l'
   Env-move (diff-in-ten eq x) = [↦…]∈.E l' [ tr (λ X → c ∈D X) (ap […]∈.δE eq) ([↦…]∈.lA l) ]≔' S


move-lΔ : ∀ {δI c d A B S}{I : Proto δI}{l : [ c ↦ A …]∈ I}{l' : [ d ↦ B …]∈ I}(df : DifferentVars… l l')
  → Proto.lookup ([ I /… l ]≔ S) ([↦…]∈.lΔ l') ≡ Env-move S df
move-lΔ {l = l} {l'} (diff-ten x)
  = lookup-diff _ ([↦…]∈.lΔ l) ([↦…]∈.lΔ l') _ x ∙ ([↦…]∈.↦Δ l')
move-lΔ {l = mk lI lE} {mk .lI lE'} (diff-in-ten refl x)
  = lookup-same _ ([]∈.lΔ lI) _ ∙  ap (λ I → I [ ↦∈.lA lE ]≔' _) ([]∈.↦Δ lI)
{-
...
move-lA : ∀ {δI c d A B S}{I : Proto δI}{l : [ c ↦ A …]∈ I}{l' : [ d ↦ B …]∈ I}(df : DifferentVars… l l')
  → Env-move S df ‼ [↦…]∈.lA l' ≡ « B »
move-lA {l = l} {l'} (diff-ten x) = [↦…]∈.↦A l'
move-lA {l = mk lI lE} {mk .lI lE'} (diff-in-ten refl x)
  =   E-lookup-diff _ x   ∙ ↦∈.↦A lE'

move… : ∀ {δI}{I : Proto δI}{c d A B S}(l : [ c ↦ A …]∈ I)(l' : [ d ↦ B …]∈ I) → DifferentVars… l l'
          → [ d ↦ B …]∈ [ I /… l ]≔ S
move… l l' df = mk4 ([↦…]∈.lΔ l') (move-lΔ df) ([↦…]∈.lA l') (move-lA df)

move-E/c : ∀ {δI} {I : Proto δI} {c d A B S} {l : [ c ↦ A …]∈ I} {l' : [ d ↦ B …]∈ I} (df : DifferentVars… l l')
  → Env.Ended ([↦…]∈.E l' [ [↦…]∈.lA l' ]≔' end)
  → Env.Ended (Env-move S df [ [↦…]∈.lA l' ]≔' end)
move-E/c {l = l} {l'} (diff-ten x) EE =  EE
move-E/c {l = mk lI lE} {mk .lI lE'} (diff-in-ten refl x) EE
  = 𝟘-elim (tr Session.Ended (E-lookup-diff _ (DiffDom-sym x) ∙ ↦∈.↦A lE) (All∈D EE (↦∈.lA lE)))
...
module _ {S S'} where
  /[>>]-/[>>]≡ : ∀ {c d δI A B}(I : Proto δI)(l : [ c ↦ A …]∈ I)(l' : [ d ↦ B …]∈ I)
      → DifferentVars… l l'
      → (I [ [↦…]∈.lΔ l >> [↦…]∈.lA l ]≔ S) [ [↦…]∈.lΔ l' >> [↦…]∈.lA l' ]≔ S'
      ≡ (I [ [↦…]∈.lΔ l' >> [↦…]∈.lA l' ]≔ S') [ [↦…]∈.lΔ l >> [↦…]∈.lA l ]≔ S
  /[>>]-/[>>]≡ · l l' (diff-ten ())
  /[>>]-/[>>]≡ (I ,[ Δ ]) (mk ⟨ ._ , ↦Δ ⟩ lE) (mk ⟨ ._ , ↦Δ₁ ⟩ lE₁) (diff-ten (t/t x))
    rewrite /[>>]-/[>>]≡ I (mk ⟨ _ , ↦Δ ⟩ lE) (mk ⟨ _ , ↦Δ₁ ⟩ lE₁) (diff-ten x) = refl
  /[>>]-/[>>]≡ (I ,[ Δ ]) (mk ⟨ .here , ↦Δ ⟩ lE) (mk ⟨ .(there l) , ↦Δ₁ ⟩ lE₁) (diff-ten (h/t l)) = refl
  /[>>]-/[>>]≡ (I ,[ Δ ]) (mk ⟨ .(there l) , ↦Δ ⟩ lE) (mk ⟨ .here , ↦Δ₁ ⟩ lE₁) (diff-ten (t/h l)) = refl
  /[>>]-/[>>]≡ I (mk ⟨ lΔ , ↦Δ ⟩ lE) (mk ⟨ .lΔ , .↦Δ ⟩ lE₁) (diff-in-ten refl x)
    = [≔][≔] _ _ (λ Δ → ≔'≔'-red Δ x) I lΔ ∙ ! [≔][≔] _ _ (λ Δ → refl) I lΔ
...
move-self : ∀ {δI c A S}(I : Proto δI)(l : [ c ↦ A ]∈ I) → [ c ↦ S ]∈ [ I / l ]≔ « S »
move-self I (mk (mk ⟨ lΔ , ↦Δ ⟩ ⟨ lA , ↦A ⟩) E/c) = mk (mk ⟨ lΔ , lookup-same I lΔ _ ⟩ ⟨ lA , E-lookup-same lA _ ⟩)
  (tr Env.Ended (! (Map.≔'≔' (Proto.lookup I lΔ) lA ∙ ap (λ X → X [ lA ]≔' end) ↦Δ)) E/c)
